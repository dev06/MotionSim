...
//		step3_cursor.GetComponent<SpriteRenderer> ().sprite = fullscreenCursor;
	...
	public void StartTutorial(bool isFullScreen)
	{
		...
		FullScreenSkip.SetActive(false);
		if (isFullScreen)
		{
			FullScreenSkip.SetActive(true);
			ReplaceAudioClips();
...
//		}
	...
	IEnumerator IntroSeq()
	{
		...
		{
			if (MergeMultiTarget.instance.trackingHandleType == MergeMultiTarget.HandleType.HideChild)
			{
				step1_scanner.GetComponent<MeshRenderer>().enabled = false;
			}
		}
...
//		}
	...
	public GameObject FullScreenSkip;
